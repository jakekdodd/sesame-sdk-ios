...
class Tests: XCTestCase {
    ...
    func testMultipleReports() {
        let sesame = MockSesame()
        let countReports = {
            BMSEventReport.fetch(context: sesame.coreDataManager.newContext(), userId: Mock.uid1)?.count
        }
...
//        for iteration in 1...desiredCount {
    ...
    func testBoot() {
        ...
        sesame.sendBoot { _ in
            sesame.coreDataManager.newContext { context in
                XCTAssert(BMSAppState.fetch(context: context, appId: sesame.appId)?.reinforcedActions.count ?? 0 > 0)
                promise.fulfill()
            }
        }
    ...
    func testReinforceDelegate() {
        ...
        sesame.sendBoot { success in
            ...
            sesame.sendReinforce(context: sesame.coreDataManager.newContext()) { success in
                XCTAssert(success)
                sesame.appLifecycleTracker.didEnterBackground()
                sesame.appLifecycleTracker.didBecomeActive()
            }
        }
